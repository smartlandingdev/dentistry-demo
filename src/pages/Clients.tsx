  ...
  const [clients] = useState<Client[]>([
    {
      ...
      nextAppointment: '20/03/2025 14:00',
      totalVisits: 12
    },
    {
      id: 2,
      ...
      nextAppointment: '25/03/2025 15:30',
      totalVisits: 8
    },
    {
      id: 3,
    ...
    },
    {
      ...
      nextAppointment: '22/03/2025 10:00',
      totalVisits: 6
    },
    {
      id: 5,
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="flex-1 relative">
          <Search className="absolute left-3 top-3 w-5 h-5 text-[#A8A29E]" strokeWidth={1.5} />
          <input
            type="text"
          ...
          filteredClients.map((client) => (
            ...
            >
              <div className="col-span-3">
                ...
                <div className="text-sm text-[#A8A29E]">{client.totalVisits} consultas</div>
              </div>
              <div className="col-span-2 flex items-center gap-2 text-[#1C1C1C]">
                <Phone className="w-4 h-4 text-[#A8A29E]" strokeWidth={1.5} />
                <span className="text-sm">{client.phone}</span>
              </div>
              <div className="col-span-2 flex items-center gap-2 text-[#1C1C1C]">
                <Mail className="w-4 h-4 text-[#A8A29E]" strokeWidth={1.5} />
                <span className="text-sm">{client.email}</span>
              </div>
              <div className="col-span-2 flex items-center gap-2 text-[#1C1C1C]">
                <Calendar className="w-4 h-4 text-[#A8A29E]" strokeWidth={1.5} />
                <span className="text-sm">{client.lastVisit}</span>
              </div>
              <div className="col-span-2 text-sm text-[#1C1C1C]">
                {client.nextAppointment ? (
                  <span className="bg-[#1C1C1C] text-[#F2EFEA] px-2 py-1 text-xs">
                    {client.nextAppointment}
                  </span>
                ) : (
              ...
              <div className="col-span-1 flex items-center justify-center gap-2">
                <button className="p-1.5 hover:bg-[#D6CBB8] transition-colors" title="Editar">
                  <Edit className="w-4 h-4 text-[#1C1C1C]" strokeWidth={1.5} />
                </button>
                <button className="p-1.5 hover:bg-[#D6CBB8] transition-colors" title="Excluir">
                  <Trash2 className="w-4 h-4 text-[#1C1C1C]" strokeWidth={1.5} />
                </button>
              </div>
            </div>
          ))
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <div className="bg-[#E8E4DF] border border-[#1C1C1C]/20 p-6">
          <div className="text-sm font-medium text-[#A8A29E] mb-2">Total de Pacientes</div>
          <div className="text-3xl font-bold text-[#1C1C1C]">{clients.length}</div>
        </div>
        <div className="bg-[#E8E4DF] border border-[#1C1C1C]/20 p-6">
          <div className="text-sm font-medium text-[#A8A29E] mb-2">Com Consulta Agendada</div>
          <div className="text-3xl font-bold text-[#1C1C1C]">
            {clients.filter(c => c.nextAppointment).length}
          </div>
        </div>
        <div className="bg-[#E8E4DF] border border-[#1C1C1C]/20 p-6">
          <div className="text-sm font-medium text-[#A8A29E] mb-2">Total de Consultas</div>
          <div className="text-3xl font-bold text-[#1C1C1C]">
